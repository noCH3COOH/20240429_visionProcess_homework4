...
bool socket_sendFrame_server(cv::Mat& frame) { 
    ...
    buffer = (unsigned char*)realloc(buffer, package_len * sizeof(unsigned char));
    std::copy(encode_data.begin(), encode_data.end(), buffer + (4 - (len % 4)));
    writeHexToFile(buffer, package_len, "log");
    ...
    if(flag_udp) {
        ...
        unsigned char* udp_recv_cache = (unsigned char*)malloc(20);
        ssize_t recv_len = -1;
        int recv_RN;
        short retry;
        bool recv_notOK;
